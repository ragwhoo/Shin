...
if (Test-Path $pluginSrc) {
    ...
    Write-Host " ✗ (plugin not found at $pluginSrc)"
}
...
if (Test-Path $skillSrc) {
    ...
    Write-Host " ✗ (skill not found at $skillSrc)"
}
...
for (`$i = 0; `$i -lt 30; `$i++) {
    ...
    try { `$r = Invoke-WebRequest -Uri "http://localhost:`$Port/api/v1/concepts" -UseBasicParsing -TimeoutSec 2
          if (`$r.StatusCode -eq 200) { Write-Host "EE ready on port `$Port"; exit 0 } } catch {}
}
...
Write-Host "Contents: ee.jar, engineering-brain/ (96 files), plugin/, skill/, start.ps1" -ForegroundColor Cyan
